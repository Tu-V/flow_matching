"""
Sampling + Hallucination Analysis cho UNet Rectified Flow (reflow, train bằng
train_shapes_fm_reflow.py) — cùng kiến trúc UNet baseline 9.1M (chỉ khác coupling
lúc train: reflow dùng cặp (x_0,x_1) gần như xác định thay vì độc lập ngẫu nhiên).

Giống hệt pipeline phân tích của sample_shapes_fm.py / sample_dit_hallucination_analysis.py
/ sample_unet_large_hallucination_analysis.py (PASS 1 bulk sample + detect hallucination,
PASS 2 trace chi tiết các case hallucination).

Source distribution: Gauss liên tục x_init = torch.randn(B,3,16,16) — đúng lý thuyết
Rectified Flow (reflow chỉ làm THẲNG đường đi, không đổi marginal 2 đầu N(0,I) <-> p_data),
và cũng đúng cách _sample_and_save trong train_shapes_fm_reflow.py đã dùng.

Mặc định lấy checkpoint epoch 50 (--ckpt), KHÔNG phải checkpoint mới nhất — vì quan sát
ban đầu cho thấy epoch 1000 sinh nhiều ảnh đen/rỗng hơn hẳn so với epoch 50 (ảnh mờ hơn
nhưng có nội dung) -> nghi ngờ reflow bị overfit vào coupling gần-xác định khi train quá
lâu (loss rơi rất nhanh, xem train_shapes_fm_reflow.py), cần so sánh % hallucination giữa
các checkpoint để xác nhận.

Usage (trên máy GPU, sau khi train_shapes_fm_reflow.py xong):
    python sample_reflow_hallucination_analysis.py                       # mặc định epoch 50, 100000 samples
    python sample_reflow_hallucination_analysis.py --ckpt shapes_fm_reflow_output/checkpoints/unet_reflow_epoch1000.pt
"""
...
# ── Default dirs (khớp với train_shapes_fm_reflow.py) ───────────────────────────
OUTPUT_DIR = os.path.join(REPO_ROOT, "shapes_fm_reflow_output")
CKPT_DIR = os.path.join(OUTPUT_DIR, "checkpoints")
DEFAULT_CKPT = os.path.join(CKPT_DIR, "unet_reflow_epoch0050.pt")
...
def build_unet() -> UNetModel:
    """Kiến trúc UNet baseline y hệt train_shapes_fm.py / train_shapes_fm_reflow.py (9.1M)."""
    return UNetModel(
...
def run(args):
    ...
    # ── dirs ── (suy ra từ checkpoint đang dùng, vd .../shapes_fm_reflow_output/checkpoints/xxx.pt
    #             -> .../shapes_fm_reflow_output/hallucination_analysis_epoch<N>)
    run_root = os.path.dirname(os.path.dirname(os.path.abspath(ckpt_path)))
    ckpt_tag = os.path.splitext(os.path.basename(ckpt_path))[0]   # vd unet_reflow_epoch0050
    analysis_dir = os.path.join(run_root, f"hallucination_analysis_{ckpt_tag}")
    ...
    with open(stats_path, "w") as f:
        f.write(f"Checkpoint : {os.path.basename(ckpt_path)}  (arch=UNet reflow, 9.1M)\n")
...
def parse_args():
    ...
    p.add_argument("--ckpt",       type=str, default=DEFAULT_CKPT,
                   help="Checkpoint path (default: epoch 50 — dùng để so sánh với epoch 1000, "
                        "vì epoch 1000 quan sát thấy nhiều ảnh đen/rỗng hơn hẳn)")
