"""
So sánh 2 phân phối p0 KHÔNG PHẢI Gaussian, KIẾN TRÚC DIT — bản tách riêng từ
train_p0_uniform_family_experiment.py để chạy song song trên máy khác với bản UNet
(train_p0_uniform_family_unet.py); mỗi script tự chạy độc lập, không cần --archs:
...
TÁI DÙNG nguyên DiT/DiTVelocityWrapper từ train_shapes_fm_dit.py và
ShapesDataset/IN_CHANNELS/size_label từ train_anisotropic_p0_experiment.py, không
viết lại. Cả 2 độ phân giải (--img_sizes 16,64) và cả 2 cỡ dataset (--dataset_sizes
5000,10000) — mặc định chạy HẾT 2(p0)×2(img_size)×2(size) = 8 tổ hợp trong 1 lần
gọi. Dataset chỉ load 1 lần cho mỗi (img_size, size), tái dùng cho cả 2 p0.

DiT mặc định dùng patch_size=2:
  - 16x16 -> grid 8x8 = 64 token (hidden=256, depth=6, heads=4 -> ~7.4M param)
  - 64x64 -> grid 32x32=1024 token (attention O(N^2) NẶNG hơn nhiều — script tự in
    cảnh báo ước lượng VRAM lúc chạy; tăng --patch_size lên 4 (256 token) hoặc giảm
    --batch_size nếu OOM).

hallucination_detector không cần sửa (đã verify đúng ở cả 16x16 và 64x64). Ảnh 1
kênh grayscale — replicate 3 kênh CHỈ lúc gọi analyze_batch.
...
from train_anisotropic_p0_experiment import (        # noqa: E402  (tái dùng, không viết lại)
    IN_CHANNELS, ShapesDataset, size_label,
)
...
OUTPUT_DIR = os.path.join(REPO_ROOT, "p0_uniform_family_dit_output")
...
def ckpt_stub(n_size: int, p0_type: str, img_size: int) -> str:
    return f"dit_n{n_size}_{p0_type}_img{img_size}"


def save_checkpoint(model, ckpt_path: str, args, optimizer=None, scheduler=None, **extra):
    state = {
...
def train_one(dataset: ShapesDataset, p0_type: str, bounds, args, device,
    ...
    print(f"\n{'='*70}")
    print(f"arch=dit  n_data={n}  p0={p0_type}  img_size={img_size}  "
    ...
    for step in range(start_step, args.total_steps + 1):
        ...
        if step % args.log_every == 0 or step == 1:
            ...
            print(f"  [dit n={n} p0={p0_type} img={img_size}] step {step:7d}/{args.total_steps}  "
        ...
        if args.save_ckpt and (step % args.save_every == 0) and step != args.total_steps:
            ...
            save_checkpoint(model, ckpt_path, args, optimizer=optimizer, scheduler=scheduler,
    ...
    if args.save_ckpt:
        final_path = os.path.join(ckpt_dir, f"{stub}.pt")
        save_checkpoint(model, final_path, args, optimizer=optimizer, scheduler=scheduler,
                         n_data=n_size, p0_type=p0_type, img_size=img_size,
...
def main():
    ...
    for isz in img_sizes:
        if isz % args.patch_size != 0:
            raise ValueError(f"img_size={isz} phải chia hết cho patch_size={args.patch_size}")
        n_tokens = (isz // args.patch_size) ** 2
        print(f"  [DiT @ img_size={isz}] n_tokens={n_tokens} (grid {isz//args.patch_size}x"
              f"{isz//args.patch_size})")
        if n_tokens > 256:
            approx_gib = args.batch_size * args.num_heads * n_tokens * n_tokens * 4 / (1024**3)
            print(f"    CẢNH BÁO OOM: attention matrix ước ~{approx_gib:.1f} GiB với "
                  f"batch_size={args.batch_size}. Tăng --patch_size hoặc giảm --batch_size "
                  f"nếu OOM (VD patch=4 -> {(isz//4)**2} token, giảm ~"
                  f"{(n_tokens/((isz//4)**2))**2:.0f}x VRAM vì O(n^2)).")
        ...
        for n_size in sizes:
            ...
            for p0_type in p0_types:
                ...
                with open(os.path.join(run_dir, "stats.txt"), "w") as f:
                    f.write(f"arch=dit  img_size={img_size}  n_data={n_size}  p0_type={p0_type}\n")
