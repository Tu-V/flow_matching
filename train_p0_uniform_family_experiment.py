"""
So sánh 2 phân phối p0 KHÔNG PHẢI Gaussian (khác hẳn N(0,I) đẳng hướng/dị hướng đã thử
ở train_anisotropic_p0_experiment.py — ở đây đổi hẳn HỌ phân phối, không chỉ đổi
covariance của Gaussian):
...
Mục tiêu: hallucination có nhạy với việc đổi HỌ phân phối p0 (uniform, có biên hữu
hạn, không có đuôi Gauss) hay không — bổ sung cho thí nghiệm Gaussian trước.

Chạy với CẢ 2 kiến trúc (--archs unet,dit) — TÁI DÙNG nguyên class đã có, không viết
lại: build_unet/UNetVelocityWrapper/ShapesDataset từ train_anisotropic_p0_experiment.py,
DiT/DiTVelocityWrapper từ train_shapes_fm_dit.py. Cả 2 độ phân giải (--img_sizes
16,64) và cả 2 cỡ dataset (--dataset_sizes 5000,10000) — mặc định chạy HẾT
2(p0)×2(arch)×2(img_size)×2(size) = 16 tổ hợp trong 1 lần gọi. Dataset chỉ load 1 lần
cho mỗi (img_size, size), tái dùng cho cả 2 arch × 2 p0 (không load lại 16 lần).
...
Checkpoint lưu ĐỊNH KỲ (--save_every) + cuối cùng, tên có arch+img_size+p0+size để
KHÔNG đụng nhau giữa các tổ hợp. Tự động RESUME nếu đã có checkpoint (--no_resume để
tắt) — bỏ qua hoàn toàn nếu đã train đủ total_steps.

CẢNH BÁO CHI PHÍ: mặc định 16 tổ hợp × total_steps (40,000) = 640,000 gradient step +
16 × 100,000 ảnh × n_repeats (5) sample. Dùng --archs/--img_sizes/--dataset_sizes/
--p0_types để chạy 1 phần trước khi chạy full.

Đã chuẩn hoá uniform_i2 theo D^2 nên KHÔNG còn tràn số như bản đầu (U[i^2,(i+1)^2]
thô cho loss ~1e13 ở 64x64) — mọi chiều đều nằm trong [0,1], quy mô giống hệt
uniform01, chỉ khác cách CHIA khoảng [0,1] cho từng chiều (đều ở uniform01, lệch
bậc 2 ở uniform_i2). Script tự in bin đầu/cuối lúc chạy để kiểm tra nhanh.

Usage:
    python train_p0_uniform_family_experiment.py
    python train_p0_uniform_family_experiment.py --archs unet --img_sizes 16
    python train_p0_uniform_family_experiment.py --archs dit --img_sizes 64 --patch_size 4
    python train_p0_uniform_family_experiment.py --dataset_sizes 5000 --total_steps 5000  # test nhanh
"""
...
from train_shapes_fm_dit import DiT, DiTVelocityWrapper   # noqa: E402  (tái dùng, không viết lại)

DATASET_ROOT = os.path.join(
    REPO_ROOT, "..", "neurips-2024-diffusion-model-hallucination", "simple-datasets",
)
OUTPUT_DIR = os.path.join(REPO_ROOT, "p0_uniform_family_output")
...
# Công thức thô U[i^2,(i+1)^2] tăng bậc 2 theo D KHÔNG chuẩn hoá (VD D=4096 ở
# 64x64 -> chiều cuối ~U[16.77 triệu]) — lệch quy mô cực đoan so với target ảnh
# [-1,1], khiến loss ~1e13 (không NaN, nhưng vô nghĩa để học/so sánh). Chuẩn hoá
# lại bằng D^2 (tổng số chiều bình phương): U[(i/D)^2, ((i+1)/D)^2] — GIỮ NGUYÊN
# hình dạng tăng bậc 2 tương đối giữa các chiều (chiều đầu bin cực hẹp quanh 0,
# chiều cuối bin rộng nhất), nhưng toàn bộ giá trị bị nén khớp về [0,1] — cùng
# thang đo với uniform01 (so sánh công bằng) và gần thang đo target ([-1,1] sau
# chuẩn hoá ảnh) hơn NHIỀU so với công thức thô, không phụ thuộc D nên không
# tràn số dù D lớn (64x64).
...
# ── Model: dispatch UNet (tái dùng) / DiT (tái dùng) ───────────────────────────
...
def build_model(arch: str, img_size: int, args):
    return build_unet() if arch == "unet" else build_dit(img_size, args)


def make_wrapper(arch: str, model):
    return UNetVelocityWrapper(model) if arch == "unet" else DiTVelocityWrapper(model)


def model_forward(arch: str, model, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    return model(x, t, extra={}) if arch == "unet" else model(x, t)


# ── Checkpoint (resume-aware) ──────────────────────────────────────────────────
def ckpt_stub(arch: str, n_size: int, p0_type: str, img_size: int) -> str:
    return f"{arch}_n{n_size}_{p0_type}_img{img_size}"
...
# ── Train 1 model (arch, n_data, p0_type, img_size) — theo STEP, resume-aware ──
def train_one(arch: str, dataset: ShapesDataset, p0_type: str, bounds, args, device,
              img_size: int, ckpt_dir: str, n_size: int):
    model = build_model(arch, img_size, args).to(device)
    ...
    stub = ckpt_stub(arch, n_size, p0_type, img_size)
    ...
    print(f"\n{'='*70}")
    print(f"arch={arch}  n_data={n}  p0={p0_type}  img_size={img_size}  params={n_params:,}  "
    ...
    for step in range(start_step, args.total_steps + 1):
        ...
        u_pred = model_forward(arch, model, path_sample.x_t, t)
        ...
        if step % args.log_every == 0 or step == 1:
            ...
            print(f"  [{arch} n={n} p0={p0_type} img={img_size}] step {step:7d}/{args.total_steps}  "
        ...
        if args.save_ckpt and (step % args.save_every == 0) and step != args.total_steps:
            ...
            save_checkpoint(model, ckpt_path, optimizer=optimizer, scheduler=scheduler,
                             arch=arch, n_data=n_size, p0_type=p0_type, img_size=img_size,
...
@torch.no_grad()
def sample_and_analyze(arch: str, model, p0_type: str, bounds, device, n_total: int,
                        steps: int, batch_size: int, img_size: int) -> dict:
    model.eval()
    wrapper = make_wrapper(arch, model)
...
def sample_and_analyze_repeated(arch: str, model, p0_type: str, bounds, device, n_total: int,
                                 steps: int, batch_size: int, n_repeats: int, img_size: int) -> dict:
    runs = []
    for rep in range(1, n_repeats + 1):
        print(f"  -- repeat {rep}/{n_repeats} --")
        s = sample_and_analyze(arch, model, p0_type, bounds, device, n_total, steps, batch_size, img_size)
        runs.append(s)
...
def parse_args():
    p = argparse.ArgumentParser(description="p0 uniform01 vs uniform_i2 — UNet & DiT, 16/64x64, 5k/10k")
    p.add_argument("--archs", type=str, default="unet,dit")
    ...
    p.add_argument("--total_steps", type=int, default=40000,
                   help="Tong so gradient step MOI to hop (arch,size,p0,img_size) — CO DINH")
    p.add_argument("--batch_size", type=int, default=128)
    p.add_argument("--lr_unet", type=float, default=2e-4)
    p.add_argument("--lr_dit", type=float, default=1e-4)
    p.add_argument("--log_every", type=int, default=2000)
    # DiT hyperparams — "nho" (~7.4M), tuong duong quy mo UNet baseline (~9.1M)
    p.add_argument("--hidden_size", type=int, default=256)
    p.add_argument("--depth", type=int, default=6)
    p.add_argument("--num_heads", type=int, default=4)
    p.add_argument("--patch_size", type=int, default=2,
                   help="Phai chia het moi img_size trong --img_sizes")
...
def main():
    args = parse_args()
    archs = [s.strip() for s in args.archs.split(",")]
    ...
    print(f"archs={archs}  img_sizes={img_sizes}  dataset_sizes={sizes}  p0_types={p0_types}")
    n_combos = len(archs) * len(img_sizes) * len(sizes) * len(p0_types)
    ...
    if "dit" in archs:
        for isz in img_sizes:
            n_tokens = (isz // args.patch_size) ** 2
            if n_tokens > 256:
                approx_gib = args.batch_size * args.num_heads * n_tokens * n_tokens * 4 / (1024**3)
                print(f"  [DiT @ img_size={isz}] n_tokens={n_tokens} -> attention matrix ước ~"
                      f"{approx_gib:.1f} GiB với batch_size={args.batch_size} -> dễ OOM. Cân nhắc "
                      f"tăng --patch_size hoặc giảm --batch_size cho img_size này.")
    ...
    for img_size in img_sizes:
        ...
        for n_size in sizes:
            ...
            for arch in archs:
                args.lr = args.lr_unet if arch == "unet" else args.lr_dit

                for p0_type in p0_types:
                    bounds = bounds_i2 if p0_type == "uniform_i2" else None

                    model = train_one(arch, dataset, p0_type, bounds, args, device,
                                       img_size, ckpt_dir, n_size)

                    print(f"\nSampling {args.n_sample_total} ảnh × {args.n_repeats} lần lặp "
                          f"(arch={arch}, img={img_size}, n={n_size}, p0={p0_type}, "
                          f"steps={args.sample_steps}) ...")
                    s = sample_and_analyze_repeated(arch, model, p0_type, bounds, device,
                                                     args.n_sample_total, args.sample_steps,
                                                     args.sample_batch_size, args.n_repeats, img_size)

                    print(f"  arch={arch:5s} img={img_size:3d} n={n_size:6d}  p0={p0_type:12s}  "
                    ...
                    results.append({"arch": arch, "img_size": img_size, "n_data": n_size,
                                     "p0_type": p0_type, **s})

                    run_dir = os.path.join(OUTPUT_DIR, ckpt_stub(arch, n_size, p0_type, img_size))
                    os.makedirs(run_dir, exist_ok=True)
                    with open(os.path.join(run_dir, "stats.txt"), "w") as f:
                        f.write(f"arch={arch}  img_size={img_size}  n_data={n_size}  "
                                f"p0_type={p0_type}\n")
    ...
    print(f"{'='*90}")
    header = (f"{'arch':>5s} {'img':>4s} {'n_data':>7s} {'p0_type':>10s} {'hall% mean':>11s} "
              f"{'hall% std':>10s} {'empty% mean':>12s} {'double_col% mean':>17s}")
    print(header)
    lines = [header]
    for r in results:
        line = (f"{r['arch']:>5s} {r['img_size']:4d} {r['n_data']:7d} {r['p0_type']:>10s} "
    ...
    if "uniform01" in p0_types and "uniform_i2" in p0_types:
        ...
        for arch in archs:
            for img_size in img_sizes:
                for n_size in sizes:
                    u01 = next((r for r in results if r["arch"] == arch and r["img_size"] == img_size
                                and r["n_data"] == n_size and r["p0_type"] == "uniform01"), None)
                    ui2 = next((r for r in results if r["arch"] == arch and r["img_size"] == img_size
