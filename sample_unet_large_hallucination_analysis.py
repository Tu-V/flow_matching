"""
Sampling + Hallucination Analysis cho UNet PHÓNG TO (~48M, train bằng
train_shapes_fm_unet_large.py).

Giống hệt pipeline phân tích của sample_shapes_fm.py / sample_dit_hallucination_analysis.py
(PASS 1 bulk sample + detect hallucination, PASS 2 trace chi tiết các case hallucination),
chỉ khác cỡ UNet (128 channels, 4 res-block, attention ở mọi level thay vì 64 channels/3
res-block/1 level attention của baseline).

Source distribution: Gauss liên tục x_init = torch.randn(B,3,16,16) — ĐÚNG như lúc
train_shapes_fm_unet_large.py (không phải tập nguồn cố định 5000 điểm).

Kiến trúc + hyperparams (model_channels/num_res_blocks/channel_mult/attention_resolutions/
num_heads) được đọc thẳng từ checkpoint (đã lưu sẵn lúc train) — không cần truyền lại qua CLI.
...
Usage (trên máy GPU, sau khi train_shapes_fm_unet_large.py xong):
    python sample_unet_large_hallucination_analysis.py --n_total 100000 --steps 100
    python sample_unet_large_hallucination_analysis.py --ckpt shapes_fm_unet_large_output/checkpoints/unet_large_epoch1000.pt
"""
...
# ── Default dirs (khớp với train_shapes_fm_unet_large.py) ──────────────────────
...
def load_model(ckpt_path: str, device: torch.device) -> UNetModel:
    ...
    model = UNetModel(
        ...
        attention_resolutions=tuple(ckpt["attention_resolutions"]),
        dropout=0.1,
    ...
    print(f"  arch=UNet(large)  model_channels={ckpt['model_channels']}  "
          f"num_res_blocks={ckpt['num_res_blocks']}  channel_mult={tuple(ckpt['channel_mult'])}  "
          f"attention_resolutions={tuple(ckpt['attention_resolutions'])}  params={n_params:,}")
    model.eval()
    return model
...
def run(args):
    ...
    # ── dirs ── (suy ra từ checkpoint đang dùng, vd .../shapes_fm_unet_large_output/checkpoints/xxx.pt
    #             -> .../shapes_fm_unet_large_output/hallucination_analysis)
    ...
    with open(stats_path, "w") as f:
        f.write(f"Checkpoint : {os.path.basename(ckpt_path)}  (arch=UNet large)\n")
...
def parse_args():
    ...
    p.add_argument("--ckpt",       type=str, default=None,
                   help="Checkpoint path (default: latest trong shapes_fm_unet_large_output)")
