"""
Sampling + Hallucination Analysis cho model train bằng train_shapes_fm_fixedsrc.py.

Giống hệt pipeline phân tích của sample_shapes_fm.py (PASS 1 bulk sample +
detect hallucination, PASS 2 trace chi tiết các case hallucination), CHỈ
KHÁC ở cách lấy x_init:

    sample_shapes_fm.py         : x_init = torch.randn(B, 3, 16, 16)         (Gauss liên tục)
    script này (fixed source)   : x_init = source_points[idx],  idx ~ Uniform({0..N_SOURCE-1})
                                   (đúng NGUỒN đã dùng lúc train_shapes_fm_fixedsrc.py — lấy
                                   chỉ số ngẫu nhiên CÓ LẶP trong tập 5000 điểm cố định)

Flow Matching sinh mẫu là DETERMINISTIC (Euler ODE, không noise injection)
=> với cùng 1 điểm nguồn (source_idx), model luôn cho ra đúng 1 ảnh. Vì vậy
mỗi trace lưu cả noise_init.pt lẫn source_idx.pt để trace lại chính xác.
...
Usage:
    python sample_fixedsrc_hallucination_analysis.py                      # 100000 samples, steps=100
    python sample_fixedsrc_hallucination_analysis.py --n_total 100000 --steps 100
    python sample_fixedsrc_hallucination_analysis.py --ckpt shapes_fm_fixedsrc_output/checkpoints/unet_epoch1000.pt
"""
...
def run(args):
    ...
    # ── fixed source points (đúng tập đã dùng lúc train) ──
    source_path = args.source_points or SOURCE_POINTS_PATH
    if not os.path.exists(source_path):
        raise FileNotFoundError(
            f"Không tìm thấy tập điểm nguồn cố định: {source_path}\n"
            f"Đây phải là đúng file fixed_source_points.pt đã dùng lúc train."
        )
    source_points = torch.load(source_path, weights_only=True).to(device)
    n_source = source_points.shape[0]
    print(f"Source distribution: Uniform over {n_source} fixed points  ({source_path})")

    # ── dirs ── (suy ra từ checkpoint đang dùng, vd .../shapes_fm_fixedsrc_output/checkpoints/xxx.pt
    #             -> .../shapes_fm_fixedsrc_output/hallucination_analysis)
    ...
    print(f"        x_init ~ Uniform(fixed source, N={n_source}), with replacement")
    print(f"{'='*60}")

    all_noises   = []  # each: [B, 3, 16, 16]  (= source_points[idx], for tracing)
    all_src_idx  = []  # each: [B]             (chỉ số trong tập 5000 điểm nguồn)
    ...
    while n_done < args.n_total:
        ...
        # x_0 ~ Uniform({0,...,n_source-1}), có lặp — ĐÚNG như lúc train.
        idx0 = torch.randint(0, n_source, (B,), device=device)
        x_init = source_points[idx0]
    ...
    all_src_idx_t = torch.cat(all_src_idx, dim=0)   # [N]
    all_finals_t  = torch.cat(all_finals, dim=0)    # [N, 3, 16, 16]

    n_unique_src = torch.unique(all_src_idx_t).numel()
    print(f"\nSource coverage: {n_unique_src}/{n_source} unique source points used "
          f"(avg reuse: {args.n_total / n_source:.1f}x)")
    ...
    if n_trace == 0:
        ...
        for case_num, global_idx in enumerate(hall_indices[:n_trace]):
            ...
            # 1) initial noise + chỉ số trong tập nguồn cố định
            torch.save(x_init_single.cpu(), os.path.join(case_dir, "noise_init.pt"))
            torch.save(torch.tensor(src_idx_single), os.path.join(case_dir, "source_idx.pt"))
            ...
            with open(report_path, "w") as f:
                ...
                print(f"Case {case_num+1}  |  sample_idx={global_idx}  |  source_idx={src_idx_single}")
                ...
                print(f"  source_idx={src_idx_single} trong {source_path}")
    ...
    with open(stats_path, "w") as f:
        ...
        f.write(f"Source     : Uniform over {n_source} fixed points ({source_path})\n")
        f.write(f"  unique source points used: {n_unique_src}/{n_source}\n")
